...
Процедура ЗаполнитьПоОстаткам() Экспорт
	...
	МоментОстатков = КонецДня(Дата);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("МоментОстатков", МоментОстатков);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Т.Сотрудник КАК Сотрудник,
	|	Т.СуммаОстаток КАК Сумма
	|ИЗ
	|	РегистрНакопления.ВКМ_ВзаиморасчетыССотрудниками.Остатки(&МоментОстатков, ) КАК Т
	|ГДЕ
	|	Т.СуммаОстаток > 0";
